  ...
  WeeklyStatsEntity call(List<FocusSessionModel> sessions, DateTime referenceDate) {
    ...
    final dayStats = days.map((d) {
      final daySessions = sessions.where((s) {
        final sd = s.startedAt ?? DateTime.now();
        return sd.year == d.year && sd.month == d.month && sd.day == d.day;
      }).toList();
      final totalMinutes = daySessions.fold<int>(0, (p, s) => p + (s.focusMinutes ?? 0));
      return DayStats(date: d, hoursStudied: totalMinutes / 60.0, pomodorosCompleted: daySessions.fold<int>(0, (p, s) => p + (s.completedPomodoros ?? 0)));
    }).toList();
    ...
    final prevTotalHours = prevDays.fold<double>(0.0, (p, d) {
      ...
      final totalMinutes = daySessions.fold<int>(0, (pp, s) => pp + (s.focusMinutes ?? 0));
